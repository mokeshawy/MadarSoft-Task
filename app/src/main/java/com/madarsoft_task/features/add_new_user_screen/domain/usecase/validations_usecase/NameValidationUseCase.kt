...
class NameValidationUseCase @Inject constructor() : BaseUseCase<AddNewUserState, ValidationResult> {

    override fun execute(input: AddNewUserState): ValidationResult {
        if (input.name.isBlank()) {
            return ValidationResult(
                isSuccessful = false,
                errorMessage = UiText.StringResource(resId = R.string.enter_name)
            )
        }
